...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self,d_model,seq_len,dropout):
        ...
        pe[:,0::2] = torch.sin(pos/torch.exp(torch.arange(0,d_model,2)*torch.log(10000)/d_model) for pos in range(0,seq_len))
        pe[:,1::2] =  torch.cos(pos/torch.exp(torch.arange(0,d_model,2)*torch.log(10000)/d_model) for pos in range(0,seq_len))
...
def build_transformer(src_vocab_size,tgt_vocab_size,src_seq_len,tgt_seq_len,d_model:int=512,N=6,h=8,dropout=0.1,d_ff = 2048):
